  ...
  <div class="playfair-scipher">
    ...
    <div id="encode">
      <h2>Кодирование</h2>
        ...
        <md-field id="codecomb">
          ...
          <md-input v-model="keyEncrypt" md-aurogrow></md-input>
        </md-field>
    ...
    <div id="decode">
      <h2>Декодирование</h2>
        ...
        <md-field id="codecomb">
          <label>ключ шифрования</label>
          <md-input v-model="keyDecrypt" md-aurogrow></md-input>
...
export default {
  ...
  methods:{
    encript(){
      ...
      let key = [...this.keyEncrypt.toUpperCase()]
      key = key.filter(symbol => symbol != symbol.toLowerCase())
      ...
      const matricies = getEncrMatrx(key)
    ...
    decript() {
      ...
      if (str.length === 0 || this.keyDecrypt.length === 0){
        this.decriptedText = 'пустая строка или неправильная матрица'
        return
      }

      let key = [...this.keyDecrypt.toUpperCase()]
      key = key.filter(symbol => symbol != symbol.toLowerCase())

      const matricies = getEncrMatrx(key)

      this.decriptedText=decriptString(matricies.matrix, str)
    }
